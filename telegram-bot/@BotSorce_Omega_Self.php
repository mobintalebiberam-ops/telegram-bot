...
yield $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "Omega Mod Now Is $m[2]"]);
   }
...
yield $MadelineProto->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "𝕆𝕟𝕝𝕚𝕟𝕒𝕞 💡"]);
  }
  if($text == '/coder' or $text == 'سازنده'){
       yield $this->messages->editMessage(['peer' => $peer,'id' => $msg_id,'message' => "By : @Botsorce️"]);
       }
    if ($text == '/status'){
